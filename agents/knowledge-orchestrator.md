...
**Publishing markdown docs into an existing page — use the publish tools, never `update_page`.**
- `confluence_publish_preflight` — READ-ONLY. Which inline comments survive, which attachments and page links stop being referenced. Run it first, every time.
- `confluence_publish_page` — renders markdown to storage, carries `<ac:inline-comment-marker>` elements across the rewrite, REFUSES if any anchor or asset would be lost, and verifies afterwards which comments went `dangling`.
- `confluence_markdown_to_storage` — pure preview; also lists the mermaid diagram filenames to render.
- `confluence_sync_attachments` — content-hash upload, skips unchanged files.

Why: inline comments bind to marker elements in the page BODY. A full-body republish leaves reviewers' threads dangling and drops `<ac:image>` diagrams, with nothing warning you first. Mermaid is NOT rendered by these tools — render SVGs in the repo's own toolchain and pass an `asset_map`. On a failed verification, roll back with `confluence_restore_version` using the version number the publish result hands back.
