...
- **Open Jira issues for the follow-up bucket?** — for each follow-up, propose a draft (project/type/summary/description). Apply only on explicit batch confirmation. Hand off to `triage-orchestrator` if the user wants the full triage workflow, or call `mcp__plugin_atlassian-suite_acendas-atlassian__jira_create_issue` directly for simple ones.
