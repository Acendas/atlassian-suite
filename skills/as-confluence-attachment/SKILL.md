...
5. **upload** → `confluence_upload_attachment(page_id, file_path, [filename, content_type, comment])`. Content type is guessed from the extension if not provided. Returns the attachment id.
   - To embed the uploaded file as an image inline: call `confluence_render_image_macro(filename)` and splice the resulting storage XML into the page via `/atlassian-suite:as-confluence-edit`.
