...
- **Scopes required:** `read:jira-work` for list/get/download; `write:jira-work` for upload; `delete:issue:jira` or classic `write:jira-work` for delete. Confirm via `/atlassian-suite:as-init` if any tool 403s.
