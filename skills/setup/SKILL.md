---
name: Setup Atlassian Suite
description: This skill should be used when the user asks to "setup atlassian", "configure atlassian", "configure jira/confluence/bitbucket", "set my atlassian credentials", "save atlassian credentials", "what credentials does the atlassian plugin need", "rotate my atlassian token", "remove atlassian credentials", or runs `/atlassian-suite:setup`. Walks the user through configuring credentials for the Acendas Atlassian Suite — file-based by default, env-var fallback for CI.
argument-hint: ""
...
# Setup the Atlassian Suite

Configure credentials for the unified Acendas Atlassian Suite MCP server (Jira Cloud + Confluence Cloud + Bitbucket Cloud). Default storage is `~/.acendas-atlassian/config.json` (mode 0600). Env vars override the file when set.
...
2. **Decide the path:**
   - **First-time setup** (no file, no env vars) → go to step 3.
   - **Adding a missing product** (e.g. file has Jira+Confluence, missing Bitbucket) → go to step 3, only ask for the missing fields.
   - **Token rotation** → ask for the new token only, call `configure_credentials` with just `atlassian_api_token` (or per-product if they use different tokens).
...
4. **Persist.** Call `mcp__acendas-atlassian__configure_credentials` with:
   ...
   The tool writes to `~/.acendas-atlassian/config.json` mode 0600 and echoes back what's stored (tokens masked).
